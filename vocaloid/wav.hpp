...
namespace vocaloid {
	namespace io {
		...
		public:
			...
			int64_t ReadData(char *bytes, uint64_t byte_length) override {
				if (IsEnd() || header_.size2 - pos_ < byte_length)return 0;
				in_.read(bytes, byte_length);
				pos_ += byte_length;
				return byte_length;
			}

			void Flush(char *bytes, uint64_t &byte_length) override {
